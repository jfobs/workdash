...
class LeaseInput(BaseModel):
    ...
    # Escalation / rent steps
    escalation_rate_annual: Decimal = Decimal("0")
    escalation_interval_months: int = 12
    rent_steps_json: Optional[str] = None
    ...
    def validate_term_or_end_date(self) -> "LeaseInput":
        ...
        if self.escalation_interval_months <= 0:
            raise ValueError("escalation_interval_months must be > 0")
        return self

...
You can also upload one or more lease contract PDFs in the sidebar and click **Extract leases from PDFs** (requires `OPENAI_API_KEY`). The extractor now uses a 5-step approach: text extraction + AI parse, direct PDF OCR/file parsing with AI, loose-JSON OCR parse fallback, OCR transcription-to-terms fallback, then deterministic regex fallback for rent/term.

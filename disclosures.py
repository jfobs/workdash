...
def _weighted_avg_rate(df: pd.DataFrame) -> float:
    if df.empty or "annual_discount_rate" not in df.columns:
        return 0.0
    first_rows = df.groupby("lease_id", as_index=False).first()
    liability_weights = df.groupby("lease_id")["begin_liability"].max().reindex(first_rows["lease_id"]).fillna(0)
    rates = first_rows["annual_discount_rate"].astype(float)
    if liability_weights.sum() == 0:
        return float(rates.mean()) if len(rates) else 0.0
    return float((rates * liability_weights.values).sum() / liability_weights.sum())
